...
const TrendingSidebar = ({ posts, trendingPeople, buttons, isVisible }: TrendingSidebarProps) => {
  ...
  return (
    <div className={`fixed bottom-0 right-0 h-full w-[300px] bg-white shadow-md transform transition-transform duration-300 ease-in-out ${isVisible ? 'translate-x-0' : 'translate-x-full'} md:translate-x-0 md:w-[300px] md:h-screen md:top-0 md:right-0 md:flex md:flex-col md:border-l md:border-gray-300 md:shadow-md md:bg-white`}>
      <div className="h-[80px] w-full flex justify-between items-center border-b border-gray-300 p-4">
        <span className="text-xl font-bold">{t('title-post')}</span>
        <a href="#" className="text-blue-500">{t('more')}</a>
      </div>
      <div className="flex flex-col overflow-y-auto h-full border-b border-gray-300">
        ...
        <div className="flex justify-between items-center border-b border-gray-300 p-4">
          <span className="text-xl font-bold">{t('title-people')}</span>
          <a href="#" className="text-blue-500">{t('more')}</a>
        </div>
        <div className="border-b border-gray-300">
